...
\caption{A series of variational distributions implicitly defined by
  ...
  Intermediate distributions (green and blue) are implicitly defined by mapping each initial point through many iterations of gradient descent on the posterior log-likelihood.
  These distributions don't have fixed parametric shapes -- their shapes depend on the shape of the posterior.}
\label{fig:cartoon}
\end{center}
\end{figure}
...
For some popular optimization methods, we show how this sequence of distributions can be used to form an unbiased estimate of a lower bound on the log marginal likelihood.
...
We tackle the first question in section \ref{sec:techintro}, showing that many popular optimization procedures yield tractable estimates of the marginal likelihood.
...
A third question, beyond the scope of this paper, is how close the variational distributions implied by various optimization rules approximate the true posterior.
...
Second, as parameters converge, their entropy estimate (and true entropy) will continue to decrease indefinitely, going to negative infinity.
However, generalization performance will usually not continue to get worse beyond a certain point.

Third, if a large number of parameters do not affect the predictive likelihood, but are still affected by a regularizer, their convergence will penalize the marginal likelihood estimate even though these parameters do not affect test set performance.
...
However, this estimator depends on an estimate of the noise variance, and does not provide a lower bound.
