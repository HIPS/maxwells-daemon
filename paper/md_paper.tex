...
\begin{align}
\log p(\data)
& \geq - \underbrace{\expectargs{q(\params)}{-\log p(\params, \data)}}_{\textnormal{\normalsize Energy $E[q]$}}
...
iteration according to a transition operation $\trans$,
%
\begin{align}
...
\end{align}
%
and let $q_t(\params)$ be the distribution over $\params_t$.

We can't evaluate $\varL[q_t]$ exactly, but we can obtain an unbiased estimator.
Since $\params_t$ is a sample from $q_t(\theta)$, $\log p(\params_t, \data)$ is
...
$S[q_0(\params)]$, and we have access to the Jacobian of the optimizer's
transition operator, $J(\params)$, then we can estimate $S[q_0(\params)]$ by
looking at the change in entropy at each iteration:
%
\begin{align}
S[q_{t+1}] - S[q_t] =
...
\begin{align}
\varL[q_T] \approx
  \underbrace{\log p(\params_t, \data)}_{\textnormal{\normalsize Energy}} +
...
Stochastic gradient descent is a popular and effective optimization procedure. 
