...
\usepackage[accepted]{aistats2016/aistats2016}
...
\aistatsauthor{David Duvenaud* \And Dougal Maclaurin* \And Ryan P. Adams}
...
\begin{abstract}
We show that unconverged stochastic gradient descent can be interpreted as sampling from a nonparametric approximate posterior distribution.
...
By tracking the change in entropy of this distribution during optimization, we give a scalable, unbiased estimate of a variational lower bound on the log marginal likelihood.
This bound can be used to optimize hyperparameters instead of cross-validation.
This Bayesian interpretation of SGD also suggests new overfitting-resistant optimization procedures, and gives a theoretical foundation for early stopping and ensembling.
...
Sampling~$\params_0$ from~$q_0$ and then applying the transition operator~$t$ times produces an exact sample~$\params_t$ from~$q_t(\params)$, by definition.
...
Because it is a lower bound, its increase means that the estimate of the marginal likelihood of the exact model is becoming \emph{more accurate}, even though the performance of the approximate model happens to be getting worse at the same time.
...
Code for all experiments in this paper is available at \href{http://github.com/HIPS/maxwells-daemon}{\texttt{github.com/HIPS/maxwells-daemon}}.
...
These problems could conceivably be addressed by ensembling, which has an interpretation as taking multiple exact independent samples from the implicit approximate posterior.
...
approximate posterior has moderate amounts of entropy.
...
We give such an interpretation for early stopping and ensembling: as sampling from a nonparametric approximation to the intractable posterior.
...
%We showed how to estimate the KL-divergence between 
%We introduced a marginal likelihood estimator based on a variational lower bound computed using these implicit distributions.
This interpretation leads naturally to a variational lower bound on the marginal likelihood.
We also gave an unbiased estimate of this lower bound by approximately tracking the entropy loss at each step of optimization.
...
%
%When a large validation set is available, cross-validation is the gold standard for choosing hyperparameters. 
Our method can be used to choose model and training hyperparameters even when a validation set is not available.
%In practice it doesn't beat the gold standard of cross-validation.
%However, our estimator doesn't require a validation set.
