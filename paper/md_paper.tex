...
$H[q_0(\params)]$, and we have access to the Jacobian of the optimizer's
transition operator then we can estimate $H[q_0(\params)]$ by looking at the
change in entropy at each iteration:
%
\begin{align}
H[q_{t+1}] - H[q_t] =
  \expectargs{q_t(\params_t)}{\log
    \left|\pderiv{\params_{t+1}}{\params_t}\right|}
  \nonumber
\end{align}
%
...
\varL[q_T] \approx
  \log p(\params_t, \data) +
  \sum_{t=0}^{T-1} \log \left|\pderiv{\params_{t+1}}{\params_t}\right| +
  H[q_0]
  \nonumber
\end{align}
%
