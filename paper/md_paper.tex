...
%
\begin{align}
\log Z & \geq \int q(\params) \log \pun(\params) d\params \underbrace{- \int q(\params) \log q(\params) d\params}_{\textnormal{\normalsize Entropy $H[q]$}}
...
\Delta H & = H_t' - H_t \\
 ...
 & \quad - \underbrace{\expect_{q(\vv|\vx)} \log \frac{ q(\vv|\vx)}{r(\vv|\vx)}}_{\textnormal{\scriptsize $\KL{q(\vv|\vx)}{q(\vv|\vx)}$}} \Bigg] \label{eq:r slack}
\end{align}
%
