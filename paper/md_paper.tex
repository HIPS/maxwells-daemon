...
Consider a general optimization procedure which initializes $\params$ according
to some distribution $q_0(\params)$ and updates $\params$ at each
iteration according to a transition operation ${\trans : \reals^D \rightarrow \reals^D}$,
%
\begin{align}
...
\end{align}
%
and let $q_t(\params)$ be the distribution over $\params_t$. We will assume that
the mapping $T$ is bijective.
...
looking at the change in entropy at each iteration, caclulated by considering
...
(Talk qualitatively about optimization and entropy reduction)
...
\begin{align}
S[q_{t+1}] - S[q_t] \approx \log \left| I - \stepsize H_t(\params_t) \right|
\end{align}
where $H_t$ is the Hessian of $-\log\subjointdist{t}{}$. Note that the Hessian does not
need to be positive definite or even non-singular.

(Mention that there are restrictions on $\stepsize$. It can't be larger than the
inverse of the smallest eigenvalue of $H$. If it was exactly that, it would
correspond to flattening to a line. Beyond that, it corresponds to folding
over. In any case, optimization is unstable near there.)
