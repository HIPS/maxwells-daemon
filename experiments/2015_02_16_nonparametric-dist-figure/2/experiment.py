...
def plot():
    ...
    CS = plt.contour(X, Y, np.exp(-Z))
    plt.clabel(CS, inline=1, fontsize=10)
    for point in aed_optimized_points:
        ax.plot(point[0], point[1], 'x')
    ax.set_xlim(xlims)
    ax.set_ylim(ylims)
    plt.savefig("true_density_with_aed_points.png")

    fig = plt.figure(0); fig.clf()
    ax = fig.add_subplot(111)
    CS = plt.contour(X, Y, np.exp(-Z))
    plt.clabel(CS, inline=1, fontsize=10)
    for point in asgd_optimized_points:
        ax.plot(point[0], point[1], 'x')
    ax.set_xlim(xlims)
    ax.set_ylim(ylims)
    plt.savefig("true_density_with_asgd_points.png")
