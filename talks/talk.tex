...
\frame[plain]{\frametitle{Inference is moving to stochastic optimization}
%\begin{columns}
%\hspace{-1cm}\begin{column}{6cm}
\begin{itemize}
\item First: Full-batch MCMC
\item Then: Variational Bayes (optimization)
\item Then: Stochastic variational inference (minibatches)
\item Then: SVI for deep GPs (neural networks)
\item Looks like training a (Bayesian) neural net by SGD
\item What's next?
\end{itemize}
%\end{column}
%\begin{column}{5cm}
...
\frame[plain]{\frametitle{Gradient Descent as Inference}
\begin{columns}
\hspace{-1cm}\begin{column}{6cm}
\begin{itemize} 
...
\begin{align*}
- \expectargs{q(\params)}{\log q(\params)} & \approx S[q_0] + \sum_{t=0}^{T-1} \log \left| J(\params_t) \right|
...
\hspace{-1cm}\begin{column}{6cm}
\begin{itemize} 
\item Inuitively: High curvature makes entropy decrease
...
\scriptsize%
	...
	\For{$t=1$ {\bfseries to} $T$}
		\State \phantom{$\entropy_{t} = \entropy_{t-1} + \log \left| \vI - \stepsize H_{t-1} \right|$}%\Comment{Update entropy}} 
...
\State {\bfseries input:}
	...
	\For{$t=1$ {\bfseries to} $T$}
		\State {\color{myfavblue}$\entropy_{t} = \entropy_{t-1} + \log \left| \vI - \stepsize H_{t-1} \right|$} %\Comment{Update entropy} 
		\State $\params_{t} = \params_{t-1} - \stepsize \gradparams$ % \Comment{Update parameters}	
   \EndFor
...
\vspace{1cm}
\begin{itemize}
\item Add entropy to likelihood to get lower bound estimate
\item Efficient implementation uses Hessian-vector products
\item Scales linearly in parameters and dataset size
\end{itemize}}

\frame[plain]{\frametitle{Experiments: Early Stopping}
\begin{columns}
\hspace{-1cm}\begin{column}{6cm}
\begin{itemize} 
\item Top: Training and test-set error on the Boston housing dataset.
\item Bottom: SGD marginal likelihood estimates.
\end{itemize}
\end{column}
\begin{column}{5cm}
\includegraphics[width=1.15\columnwidth]{../experiments/2015_03_01_housing/2/marglik.pdf}
...
\frame[plain]{\frametitle{Experiments: Number of Hidden Units}
\begin{columns}
\hspace{-1cm}\begin{column}{6cm}
\begin{itemize} 
\item Top: Likelihood vs hidden units on MNIST
...
\hspace{-1cm}\begin{column}{6cm}
\begin{itemize} 
\item Entropy term gets arbitrarily bad due to concentration, but true performance only gets as bad as MLE
...
\frame[plain]{\frametitle{Experiments: Entropy-friendly Optimization}
\begin{columns}
\hspace{-1cm}\begin{column}{6cm}
\begin{itemize} 
\item Modified SGD to move slower near convergence
\item Hurts performance, but gives higher bound estimate
\end{itemize}
\end{column}
\begin{column}{5cm}
...
\frame[plain]{\frametitle{More Limitations}
\begin{columns}
\hspace{-1cm}\begin{column}{6cm}
\begin{itemize} 
...
\item Early stopping chooses among these distributions, ensembling samples from them
